import { StyleSheet } from 'react-native';

const styles = StyleSheet.create({
  mainContainer: {
    flex: 1,
    backgroundColor: '#F5F7FA',
  },
  container: {
    flex: 1,
    backgroundColor: '#F5F7FA',
  },
  loadingContainer: {
    position: 'absolute',
    top: 0,
    left: 0,
    right: 0,
    bottom: 0,
    justifyContent: 'center',
    alignItems: 'center',
    backgroundColor: 'rgba(255, 255, 255, 0.8)',
    zIndex: 999,
  },
  calendarContainer: {
    padding: 12,
    marginTop: 15,
    backgroundColor: '#FFFFFF',
    borderRadius: 20,
    marginHorizontal: 16,
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.15,
    shadowRadius: 8,
    elevation: 5,
    borderWidth: 1,
    borderColor: '#EFEFEF',
  },
  legendContainer: {
    marginTop: 25,
    marginBottom: 15,
    marginHorizontal: 20,
    padding: 15,
    backgroundColor: '#FFFFFF',
    borderRadius: 16,
    borderWidth: 1,
    borderColor: '#EFEFEF',
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.1,
    shadowRadius: 4,
    elevation: 3,
  },
  legendTitle: {
    fontWeight: '700',
    fontSize: 16,
    marginBottom: 10,
    color: '#333333',
  },
  legendItemsContainer: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    flexWrap: 'wrap',
  },
  legendItem: {
    flexDirection: 'row',
    alignItems: 'center',
    marginRight: 15,
    marginVertical: 5,
  },
  legendDot: {
    width: 12,
    height: 12,
    borderRadius: 6,
    marginRight: 8,
  },
  legendText: {
    fontSize: 14,
    color: '#555555',
  },
  modalContainer: {
    flex: 1,
    justifyContent: 'center',
    alignItems: 'center',
    backgroundColor: 'rgba(0, 0, 0, 0.6)',
  },
  modalContent: {
    backgroundColor: 'white',
    borderRadius: 24,
    padding: 24,
    width: '92%',
    maxHeight: '85%',
    maxWidth: 450,
    flex: 1,
  },
  modalHeader: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    marginBottom: 18,
    paddingBottom: 12,
    borderBottomWidth: 1,
    borderBottomColor: '#EEEEEE',
  },
  modalTitle: {
    fontSize: 18,
    fontWeight: 'bold',
    color: '#1E1E1E',
    flex: 1,
  },
  todayBadge: {
    backgroundColor: '#3F51B5',
    borderRadius: 12,
    paddingVertical: 5,
    paddingHorizontal: 10,
  },
  todayText: {
    color: 'white',
    fontWeight: 'bold',
    fontSize: 12,
  },
  eventListContainer: {
    marginBottom: 15,
  },
  eventItem: {
    padding: 14,
    borderWidth: 1,
    borderColor: '#EEEEEE',
    borderRadius: 12,
    marginVertical: 7,
    backgroundColor: '#FFFFFF',
  },
  eventHeader: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
    marginBottom: 8,
  },
  eventTypeContainer: {
    flexDirection: 'row',
    alignItems: 'center',
  },
  eventTypeIndicator: {
    width: 10,
    height: 10,
    borderRadius: 5,
    marginRight: 6,
  },
  eventType: {
    fontWeight: '600',
    fontSize: 12,
    color: '#666666',
  },
  eventDetailsContainer: {
    marginTop: 4,
  },
  eventName: {
    fontSize: 15,
    fontWeight: '500',
    color: '#333333',
    marginBottom: 6,
  },
  eventInfoRow: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
  },
  eventInfoColumn: {
    flexDirection: 'column',
  },
  eventCategory: {
    fontSize: 12,
    color: '#666666',
    marginBottom: 2,
  },
  eventFrequency: {
    fontSize: 11,
    color: '#757575',
    fontStyle: 'italic',
  },
  eventAmount: {
    fontSize: 15,
    fontWeight: '700',
    textAlign: 'right',
  },
  deleteButton: {
    backgroundColor: '#F44336',
    padding: 8,
    borderRadius: 20,
    height: 36,
    width: 36,
    justifyContent: 'center',
    alignItems: 'center',
  },
  webDeleteButton: {
    backgroundColor: '#F44336',
    padding: 8,
    borderRadius: 20,
    height: 36,
    width: 36,
    justifyContent: 'center',
    alignItems: 'center',
  },
  noEventsContainer: {
    alignItems: 'center',
    justifyContent: 'center',
    paddingVertical: 30,
    marginBottom: 20,
  },
  noEventsText: {
    textAlign: 'center',
    color: '#666',
    fontSize: 16,
    marginTop: 10,
  },
  eventSummaryContainer: {
    backgroundColor: '#F8F9FA',
    borderRadius: 12,
    padding: 16,
    marginBottom: 15,
    borderLeftWidth: 4,
    borderLeftColor: '#3F51B5',
  },
  eventSummaryTitle: {
    fontWeight: '700',
    fontSize: 16,
    marginBottom: 10,
    color: '#333333',
  },
  eventSummaryContent: {
    gap: 8,
  },
  summaryRow: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
  },
  summaryLabel: {
    fontSize: 14,
    color: '#555555',
  },
  summaryValue: {
    fontSize: 15,
    fontWeight: 'bold',
  },
  buttonRow: {
    flexDirection: 'row',
    justifyContent: 'center',
    marginTop: 5,
    width: '100%',
  },
  closeButton: {
    padding: 12,
    backgroundColor: '#3F51B5',
    borderRadius: 12,
    alignItems: 'center',
    marginTop: 16,
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.2,
    shadowRadius: 3,
    elevation: 3,
    width: '100%',
  },
  closeButtonText: {
    color: 'white',
    fontWeight: 'bold',
    fontSize: 16,
  },
  addButtonsContainer: {
    marginTop: 20,
    flexDirection: 'row',
    justifyContent: 'space-between',
    gap: 12,
  },
  addButton: {
    flex: 1,
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
    padding: 14,
    borderRadius: 10,
    gap: 8,
    elevation: 3,
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.15,
    shadowRadius: 3,
  },
  addButtonText: {
    color: 'white',
    fontWeight: 'bold',
    fontSize: 14,
  },
  
  // Record selector styles
  recordSelectorContainer: {
    flex: 1,
    height: '100%',
    flexDirection: 'column',
  },
  recordSelectorHeader: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
    marginBottom: 15,
    paddingBottom: 10,
    borderBottomWidth: 1,
    borderBottomColor: '#EEEEEE',
    flexShrink: 0,
  },
  recordSelectorTitle: {
    fontSize: 18,
    fontWeight: 'bold',
    color: '#333',
  },
  closeSelector: {
    padding: 5,
  },
  searchInput: {
    backgroundColor: '#F5F5F5',
    borderWidth: 1,
    borderColor: '#EEEEEE',
    borderRadius: 10,
    padding: 12,
    marginBottom: 15,
    fontSize: 14,
    flexShrink: 0,
  },
  recordsList: {
    flex: 1,
    minHeight: 0,
  },
  recordsListContent: {
    paddingBottom: 20,
    flexGrow: 1,
  },
  recordItem: {
    padding: 12,
    borderWidth: 1,
    borderColor: '#EEEEEE',
    borderRadius: 10,
    marginBottom: 8,
    backgroundColor: '#FFFFFF',
  },
  recordText: {
    fontSize: 14,
    fontWeight: '500',
    color: '#333',
    marginBottom: 6,
  },
  recordInfoRow: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
  },
  recordInfoColumn: {
    flexDirection: 'column',
  },
  recordCategory: {
    fontSize: 12,
    color: '#666666',
    marginBottom: 2,
  },
  recordFrequency: {
    fontSize: 11,
    color: '#757575',
    fontStyle: 'italic',
  },
  recordAmount: {
    fontSize: 14,
    fontWeight: '600',
    textAlign: 'right',
  },
  noRecordsContainer: {
    flex: 1,
    alignItems: 'center',
    justifyContent: 'center',
    paddingVertical: 30,
    minHeight: 200,
  },
  noRecordsText: {
    fontSize: 16,
    color: '#666',
    marginBottom: 15,
  },
  createNewButton: {
    backgroundColor: '#3F51B5',
    padding: 14,
    borderRadius: 10,
    alignItems: 'center',
    minWidth: 200,
    elevation: 3,
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.15,
    shadowRadius: 3,
  },
  createNewButtonText: {
    color: 'white',
    fontWeight: 'bold',
    fontSize: 14,
  },
  backButton: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
    backgroundColor: '#3F51B5',
    padding: 14,
    borderRadius: 10,
    marginTop: 15,
    gap: 8,
    elevation: 3,
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.2,
    shadowRadius: 2,
    flexShrink: 0,
  },
  backButtonText: {
    color: 'white',
    fontWeight: 'bold',
    fontSize: 14,
  },
  
  // Monthly Stats Section Styles
  monthlyStatsContainer: {
    marginHorizontal: 16,
    marginVertical: 20,
    backgroundColor: '#FFFFFF',
    borderRadius: 20,
    padding: 18,
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.1,
    shadowRadius: 8,
    elevation: 4,
    borderWidth: 1,
    borderColor: '#EFEFEF',
  },
  monthlyStatsTitle: {
    fontSize: 18,
    fontWeight: '700',
    marginBottom: 15,
    color: '#333333',
    textAlign: 'center',
  },
  statsCards: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    marginBottom: 20,
    gap: 12,
  },
  statsCard: {
    flex: 1,
    borderRadius: 16,
    padding: 12,
    alignItems: 'center',
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.1,
    shadowRadius: 4,
    elevation: 3,
    minHeight: 110,
    justifyContent: 'center',
  },
  statsLabel: {
    fontSize: 12,
    marginTop: 8,
    marginBottom: 4,
    color: '#666666',
    fontWeight: '500',
    textAlign: 'center',
  },
  statsValue: {
    fontSize: 16,
    fontWeight: 'bold',
  },
  upcomingContainer: {
    marginTop: 10,
    borderTopWidth: 1,
    borderTopColor: '#EEEEEE',
    paddingTop: 15,
  },
  upcomingTitle: {
    fontSize: 17,
    fontWeight: '600',
    marginBottom: 12,
    color: '#333333',
  },
  upcomingEvent: {
    flexDirection: 'row',
    padding: 12,
    marginBottom: 10,
    backgroundColor: '#FFFFFF',
    borderRadius: 12,
    borderWidth: 1,
    borderColor: '#EEEEEE',
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.1,
    shadowRadius: 3,
    elevation: 2,
  },
  upcomingDate: {
    width: 50,
    height: 50,
    borderRadius: 10,
    backgroundColor: '#3F51B5',
    justifyContent: 'center',
    alignItems: 'center',
    marginRight: 15,
  },
  upcomingDay: {
    fontSize: 18,
    fontWeight: 'bold',
    color: '#FFFFFF',
  },
  upcomingMonth: {
    fontSize: 10,
    fontWeight: '500',
    color: '#FFFFFF',
  },
  upcomingDetail: {
    flex: 1,
    justifyContent: 'space-between',
  },
  upcomingName: {
    fontSize: 15,
    fontWeight: '600',
    color: '#333333',
    marginBottom: 4,
  },
  upcomingMetadata: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
  },
  upcomingRecurring: {
    fontSize: 12,
    color: '#757575',
    fontStyle: 'italic',
  },
  upcomingAmount: {
    fontSize: 15,
    fontWeight: '700',
  },
  noUpcomingEvents: {
    alignItems: 'center',
    justifyContent: 'center',
    paddingVertical: 20,
    backgroundColor: '#F5F7FA',
    borderRadius: 12,
    borderWidth: 1,
    borderColor: '#EEEEEE',
  },
  noUpcomingText: {
    color: '#757575',
    fontSize: 14,
    textAlign: 'center',
  },
  // Estilos para opções de recorrência
  recurringOptionContainer: {
    backgroundColor: '#F5F7FA',
    borderRadius: 12,
    padding: 15,
    marginBottom: 15,
    borderWidth: 1,
    borderColor: '#EEEEEE',
    flexShrink: 0,
  },
  recurringCheckboxRow: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    marginBottom: 10,
  },
  recurringLabel: {
    fontSize: 15,
    fontWeight: '500',
    color: '#333333',
  },
  checkbox: {
    width: 22,
    height: 22,
    borderRadius: 4,
    borderWidth: 2,
    borderColor: '#3F51B5',
    justifyContent: 'center',
    alignItems: 'center',
  },
  checkboxChecked: {
    backgroundColor: '#3F51B5',
  },
  recurringNoteContainer: {
    backgroundColor: '#E8EAF6',
    borderRadius: 8,
    padding: 10,
    marginTop: 5,
  },
  frequencyNote: {
    fontSize: 13,
    color: '#333',
    fontWeight: '500',
  },
  frequencySubNote: {
    fontSize: 12,
    fontStyle: 'italic',
    color: '#666666',
    marginTop: 5,
  },
  // Compact recurring container styles
  compactRecurringContainer: {
    backgroundColor: '#E8EAF6',
    borderRadius: 8,
    padding: 10,
    marginTop: 8,
  },
  recurrenceInputRow: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    marginBottom: 6,
  },
  recurrenceControlsContainer: {
    flexDirection: 'row',
    alignItems: 'center',
    backgroundColor: '#FFFFFF',
    borderRadius: 8,
    borderWidth: 1,
    borderColor: '#D0D0D0',
    overflow: 'hidden',
  },
  countButton: {
    backgroundColor: '#F8F9FA',
    paddingHorizontal: 8,
    paddingVertical: 8,
    justifyContent: 'center',
    alignItems: 'center',
    minWidth: 32,
    borderWidth: 0,
  },
  countButtonDisabled: {
    backgroundColor: '#F0F0F0',
  },
  recurrenceInputLabel: {
    fontSize: 13,
    color: '#333',
    fontWeight: '500',
  },
  recurrenceInput: {
    backgroundColor: '#FFFFFF',
    borderWidth: 0,
    borderColor: 'transparent',
    borderRadius: 0,
    paddingHorizontal: 12,
    paddingVertical: 8,
    fontSize: 14,
    fontWeight: '600',
    textAlign: 'center',
    minWidth: 40,
    maxWidth: 50,
  },
  recurrenceInputAtLimit: {
    backgroundColor: '#FFF8E1',
  },
  maxLimitText: {
    fontSize: 11,
    color: '#757575',
    fontStyle: 'italic',
  },
  compactFrequencyNote: {
    fontSize: 11,
    color: '#666666',
    textAlign: 'center',
    fontStyle: 'italic',
  },
  
  // Estilo para eventos filtrados
  filteredEventsContainer: {
    position: 'absolute',
    top: '5%',
    left: '5%',
    right: '5%',
    bottom: '5%',
    backgroundColor: 'white',
    zIndex: 100,
    padding: 15,
    borderRadius: 16,
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 5 },
    shadowOpacity: 0.3,
    shadowRadius: 10,
    elevation: 8,
  },
  filteredEventsHeader: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
    marginBottom: 15,
    borderBottomWidth: 1,
    borderBottomColor: '#EEEEEE',
    paddingBottom: 12,
  },
  filteredEventsTitle: {
    fontSize: 18,
    fontWeight: 'bold',
    color: '#333',
  },
  filteredEventsSubtitle: {
    fontSize: 13,
    color: '#757575',
    marginTop: 2,
  },
  filteredEventsListContent: {
    paddingBottom: 70,
  },
  closeFilterButton: {
    padding: 5,
    height: 32,
    width: 32,
    borderRadius: 16,
    justifyContent: 'center',
    alignItems: 'center',
    backgroundColor: '#f2f2f2',
  },
  // Estilos para eventos filtrados melhorados
  filteredEventItem: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
    backgroundColor: '#FFFFFF',
    padding: 12,
    borderRadius: 12,
    marginVertical: 6,
    borderWidth: 1,
    borderColor: '#EFEFEF',
    borderLeftWidth: 4,
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.1,
    shadowRadius: 3,
    elevation: 1,
  },
  filteredEventContent: {
    flex: 1,
    paddingRight: 8,
  },
  filteredEventTypeContainer: {
    flexDirection: 'row',
    alignItems: 'center',
    marginBottom: 6,
  },
  filteredEventDot: {
    width: 10,
    height: 10,
    borderRadius: 5,
    marginRight: 6,
  },
  filteredEventTypeText: {
    fontSize: 11,
    fontWeight: '700',
    color: '#555555',
    textTransform: 'uppercase',
    letterSpacing: 0.5,
  },
  filteredEventName: {
    fontSize: 15,
    fontWeight: '600',
    color: '#333333',
    marginBottom: 6,
  },
  filteredEventRecurring: {
    fontSize: 11,
    paddingVertical: 2,
    paddingHorizontal: 6,
    borderRadius: 10,
    alignSelf: 'flex-start',
    marginBottom: 2,
  },
  recurringText: {
    color: '#3F51B5',
    fontStyle: 'italic',
    backgroundColor: '#E8EAF6',
    fontWeight: '500',
  },
  oneTimeText: {
    color: '#757575',
    backgroundColor: '#F5F5F5',
  },
  deleteButtonFiltered: {
    backgroundColor: '#F44336',
    padding: 8,
    borderRadius: 20,
    height: 36,
    width: 36,
    alignItems: 'center',
    justifyContent: 'center',
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 1 },
    shadowOpacity: 0.2,
    shadowRadius: 2,
    elevation: 1,
  },
  deleteButtonText: {
    color: 'white',
    fontSize: 12,
    fontWeight: '600',
    letterSpacing: 0.5,
  },
  filteredEventMetadataRow: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
    marginTop: 6,
    borderTopWidth: 1,
    borderTopColor: '#F0F0F0',
    paddingTop: 6,
  },
  filteredEventAmount: {
    fontSize: 15,
    fontWeight: '700',
  },
  // Adicionar estilos para o sumário simplificado
  simplifiedSummaryContainer: {
    backgroundColor: '#F8F9FA',
    borderRadius: 12,
    padding: 12,
    marginBottom: 15,
    borderLeftWidth: 4,
    borderLeftColor: '#3F51B5',
    alignItems: 'flex-start',
  },
  simplifiedSummaryText: {
    fontSize: 16,
    color: '#333333',
    fontWeight: '500',
  },
  // Adicionar estilo para a frequência nos itens de registro
  recordFrequency: {
    fontSize: 11,
    color: '#757575',
    fontStyle: 'italic',
    marginTop: 4,
  },
  
  // Estilos específicos para o modal principal de eventos (compacto)
  mainEventModalContent: {
    backgroundColor: 'white',
    borderRadius: 24,
    padding: 16,
    width: '92%',
    maxHeight: '70%',
    maxWidth: 450,
  },
  mainEventModalHeader: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    marginBottom: 12,
    paddingBottom: 8,
    borderBottomWidth: 1,
    borderBottomColor: '#EEEEEE',
  },
  mainEventListContainer: {
    marginBottom: 8,
    maxHeight: 200, // Limit height to prevent pushing close button down
    flex: 0, // Don't allow it to grow beyond maxHeight
  },
  mainEventListContent: {
    paddingBottom: 8, // Add some padding at the bottom for scroll
  },
  mainEventAddButtonsContainer: {
    marginTop: 12,
    marginBottom: 8,
    flexDirection: 'row',
    justifyContent: 'space-between',
    gap: 12,
  },
  mainEventAddButton: {
    flex: 1,
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
    padding: 12,
    borderRadius: 10,
    gap: 8,
    elevation: 3,
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.15,
    shadowRadius: 3,
  },
  mainEventCloseButton: {
    padding: 12,
    backgroundColor: '#3F51B5',
    borderRadius: 12,
    alignItems: 'center',
    marginTop: 8,
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.2,
    shadowRadius: 3,
    elevation: 3,
    width: '100%',
  },
  mainEventNoEventsContainer: {
    alignItems: 'center',
    justifyContent: 'center',
    paddingVertical: 16,
    marginBottom: 12,
  },
  mainEventSimplifiedSummaryContainer: {
    backgroundColor: '#F8F9FA',
    borderRadius: 12,
    padding: 8,
    marginBottom: 8,
    borderLeftWidth: 4,
    borderLeftColor: '#3F51B5',
    alignItems: 'flex-start',
  },
  
  // Delete Modal Styles
  deleteModalContainer: {
    flex: 1,
    justifyContent: 'center',
    alignItems: 'center',
    backgroundColor: 'rgba(0, 0, 0, 0.6)',
  },
  deleteModalContent: {
    backgroundColor: 'white',
    borderRadius: 16,
    padding: 24,
    width: '85%',
    maxWidth: 380,
    alignItems: 'center',
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 5 },
    shadowOpacity: 0.3,
    shadowRadius: 8,
    elevation: 8,
  },
  deleteModalHeader: {
    alignItems: 'center',
    marginBottom: 20,
  },
  deleteModalTitle: {
    fontSize: 20,
    fontWeight: 'bold',
    color: '#333',
    marginTop: 8,
  },
  deleteModalMessage: {
    fontSize: 16,
    color: '#555',
    marginBottom: 25,
    textAlign: 'center',
    lineHeight: 22,
  },
  deleteModalButtonsContainer: {
    width: '100%',
    alignItems: 'center',
  },
  deleteModalAllOccurrencesButton: {
    backgroundColor: '#ffebee',
    width: '100%',
    padding: 15,
    borderRadius: 12,
    alignItems: 'center',
    marginBottom: 12,
    borderWidth: 1,
    borderColor: '#ffcdd2',
  },
  deleteModalSingleOccurrenceButton: {
    backgroundColor: '#e8eaf6',
    width: '100%',
    padding: 15,
    borderRadius: 12,
    alignItems: 'center',
    marginBottom: 12,
    borderWidth: 1,
    borderColor: '#c5cae9',
  },
  deleteModalDeleteButton: {
    backgroundColor: '#ffebee',
    width: '100%',
    padding: 15,
    borderRadius: 12,
    alignItems: 'center',
    marginBottom: 12,
    borderWidth: 1,
    borderColor: '#ffcdd2',
  },
  deleteModalCancelButton: {
    backgroundColor: '#f5f5f5',
    width: '100%',
    padding: 15,
    borderRadius: 12,
    alignItems: 'center',
  },
  deleteModalButtonTextDanger: {
    color: '#d32f2f',
    fontWeight: 'bold',
    fontSize: 15,
  },
  deleteModalButtonTextPrimary: {
    color: '#3F51B5',
    fontWeight: 'bold',
    fontSize: 15,
  },
  deleteModalButtonTextCancel: {
    color: '#757575',
    fontWeight: 'bold',
    fontSize: 15,
  },
});

export default styles; 